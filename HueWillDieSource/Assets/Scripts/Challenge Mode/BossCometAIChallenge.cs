...
public class BossCometAIChallenge : MonoBehaviour {
	...
	private int chargingTimeFinal = 49;
	private int ShootInterval = 8;
	...
	void FixedUpdate () {
		if (waitTime <= 0) {
			...
			if (state == 2) {
				...
				if (waitTime % ShootInterval == 0) {
					forwardAngle = FindAngleTowardsPlayer (gameObject.transform.position);
					currentBullet = getNextBullet (4, 7);
					TargetedShootAtAngle (currentBullet, 10, 6, 3.5f, gameObject.transform.position, forwardAngle);
					TargetedShootAtAngle (currentBullet, 10, 6, 3.5f, gameObject.transform.position, (forwardAngle + 120.0f) % 360.0f);
					TargetedShootAtAngle (currentBullet, 10, 6, 3.5f, gameObject.transform.position, (forwardAngle - 120.0f) % 360.0f);
				}
			} else if (state == 3) {
				forwardAngle = FindAngleTowardsPlayer (gameObject.transform.position);
				...
				if (waitTime % 45 == 0) {
					...
					TargetedShootAtAngle (currentBullet, 3, 30, 3.0f, leftPos, FindAngleTowardsPlayer(leftPos) + 30.0f);
					TargetedShootAtAngle (currentBullet, 3, 30, 3.0f, rightPos, FindAngleTowardsPlayer(rightPos) - 30.0f);
				}
			}
